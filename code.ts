...
async function scanLocalVariables(
  collectionId?: string
): Promise<{ name: string; rgba: string; modes: string[]; aliasName?: string; variableCollectionId: string }[]> {
  try {
    const localVariables = await figma.variables.getLocalVariablesAsync();
    const localCollections = await figma.variables.getLocalVariableCollectionsAsync();

    const collectionModes: { [collectionId: string]: { [modeId: string]: string } } = {};
    ...
    const colorVariables: { name: string; rgba: string; modes: string[]; aliasName?: string; variableCollectionId: string }[] = [];
